...
LABEL maintainer="Jefferson Silva Jeffferson.admsistemas@gmail.com"
...
RUN buildDeps='curl gcc make autoconf libc-dev zlib1g-dev pkg-config' \
    ...
    && apt-get install --no-install-recommends --no-install-suggests -q -y \
            ...
            php7.1-xml \
            php-pear \
    && pecl -d php_suffix=7.1 install -o -f redis memcached imagick \
    && mkdir -p /run/php \
    && pip install wheel \
    ...
    && ln -sf /etc/php/7.1/mods-available/imagick.ini /etc/php/7.1/fpm/conf.d/20-imagick.ini \
